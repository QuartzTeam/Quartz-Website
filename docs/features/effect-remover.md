...
- **Simple** — turns off entire effect categories at runtime without touching the chart: **Disable Filters** (Grayscale, Arcade, etc.), **Disable Bloom**, **Disable Flash**, **Disable Hall of Mirrors**, **Disable Screen Shake**, and **Max Tile Movements** to cap how far a single Move Track event can shove tiles.
...
!!! warning "Enhanced mode blocks saving by default"
    While Enhanced is stripping events, the editor is holding a modified copy of the level — saving would overwrite your original chart with the stripped version. Turn on **Allow Saving in Editor** if you actually want that; otherwise saving stays blocked.
