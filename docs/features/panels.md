...
Open the Quartz menu in-game, go to the **Overlay** tab, and open the **Panels** section.

## Highlights

- **Create Panel** — makes a new empty panel; name it, then use **+ Add Stat** to pick from every stat Quartz exposes (accuracy, X-accuracy, KPS, BPM (current/target), progress, timing scale, holds, checkpoints, attempt/best/total-attempts, map time, music time, pitch, FPS, XPerfect counts, or a custom **Text** stat you write yourself).
...
- **Per-stat color** — tint an individual stat's value with a gradient across its own 0–100% range, with an optional override color for a perfect (100%) result.
